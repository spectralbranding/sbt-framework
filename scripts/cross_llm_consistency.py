...
DIMS = ["semiotic","narrative","ideological","experiential","social","economic","cultural","temporal"]
...
def call_anthropic(brand):
    ...
    r = c.messages.create(model="claude-sonnet-4-6", max_tokens=4000, temperature=0,
        system=SYSTEM, messages=[{"role":"user","content":INSTR.format(brand=brand)}])
...
def call_openai(brand):
    ...
    r = c.chat.completions.create(model="gpt-4o", temperature=0,
        messages=[{"role":"system","content":SYSTEM},{"role":"user","content":INSTR.format(brand=brand)}])
...
def call_gemini(brand):
    ...
    r = c.models.generate_content(model="gemini-2.5-pro",
        contents=SYSTEM+"\n\n"+INSTR.format(brand=brand),
        config=types.GenerateContentConfig(temperature=0))
    return r.text

MODELS = {"claude-sonnet-4-6": call_anthropic, "gpt-4o": call_openai, "gemini-2.5-pro": call_gemini}
...
for brand in BRANDS:
    ...
    for mname, fn in MODELS.items():
        try:
            ...
            p = parse(txt)
            if p: results[brand][mname] = p; print(f"OK  {brand:10} {mname:18} strength={p[0]}")
            else: print(f"PARSE-FAIL {brand} {mname}")
...
for brand in BRANDS:
    ...
    if len(ms) < 2:
        print(f"{brand}: <2 models, skip"); continue
    frs, sps = [], []
    for a,b in itertools.combinations(ms,2):
        ...
        frs.append(fr); sps.append(sp)
        print(f"  {brand:10} {a:16} vs {b:16}: Fisher-Rao(signature)={fr:.4f}  Spearman(strength)={sp:.3f}")
    all_fr += frs; all_sp += sps
    print(f"  {brand:10} mean Fisher-Rao={np.mean(frs):.4f}  mean Spearman={np.mean(sps):.3f}")
if all_fr:
    print(f"\nOVERALL mean pairwise Fisher-Rao = {np.mean(all_fr):.4f} (range {min(all_fr):.4f}-{max(all_fr):.4f})")
    print(f"OVERALL mean pairwise Spearman   = {np.mean(all_sp):.3f} (range {min(all_sp):.3f}-{max(all_sp):.3f})")
